...
EPrints::Plugin::InputForm::Component::HiddenDocuments

=cut

package EPrints::Plugin::InputForm::Component::HiddenDocuments;
...
sub update_from_form
{
	...
	# cache documents
	$eprint->set_value( "hidden_documents", $eprint->value( "hidden_documents" ) );
	my @eprint_docs = $eprint->get_all_hidden_documents;
	...
	if( $session->internal_button_pressed )
	{
		...
		if( $internal && $internal =~ m/^doc(\d+)_(.+)$/ )
		{
			...
			my $doc = $session->dataset( "hidden_document" )->dataobj( $docid );
...
sub get_state_fragment
{
	...
	my $eprint = $self->{workflow}->{item};
	foreach my $doc ( $eprint->get_all_hidden_documents )
	{
		my $doc_prefix = $self->{prefix}."_doc".$doc->id;
...
sub _reorder
{
	...
	my @docs = $self->{workflow}->{item}->get_all_hidden_documents;
...
sub get_fields_handled { qw( hidden_documents ) }
...
sub render_content
{
	...
	my @docs = $eprint->get_all_hidden_documents;
...
sub export
{
	...
	else
	{
		...
		my $doc = $self->{session}->dataset( "hidden_document" )->dataobj( $docid );
...
sub validate
{
	...
	push @problems, $eprint->validate_field( "hidden_documents" );
	...
	if( !$ok )
	{
		my $doc_ds = $session->dataset( "hidden_document" );
...
sub parse_config
{
	...
	my $doc_ds = $self->{session}->get_dataset( "hidden_document" );
