...
EPrints::Plugin::Screen::EPrint::UploadMethod::HiddenFile

=cut

package EPrints::Plugin::Screen::EPrint::UploadMethod::HiddenFile;
...
sub new
{
	...
	return $self->SUPER::new(
		appears => [
			{ place => "hidden_upload_methods", position => 200 },
		],
...
sub action_add_format
{
    ...
    my $list;
	my $doc = $eprint->create_subdataobj( "hidden_documents", $epdata );
	if( defined $doc )
	{
	...
	{
		$processor->add_message( "error", $session->html_phrase( "Plugin/InputForm/Component/HiddenUpload:create_failed" ) );
		return;
	}
...
sub action_create_file
{
	...
	my $doc = $eprint->create_subdataobj( "hidden_documents", {
			main => $filename,
			mime_type => $mime_type,
			format => "other",
		});
